...
export class SequelizeUserRepositoryAdapter implements UserRepositoryPort {
  constructor(@Inject(UserRepository) private readonly userRepository: typeof UserModel) { }

  async getUser(id: number): Promise<User> {
    return await this.userRepository.findOne<UserModel>({ where: { id } }).then(user => {
      if (user !== null) {
        return new User(modelToJson(user))
      } else {
        throw new HttpException('Usuario por este Id nao encontrado', HttpStatus.BAD_REQUEST)
      }
    }).catch(error => {
      throw new HttpException('Parametros invalidos, ou incompletos', HttpStatus.BAD_REQUEST, { cause: error });
    })
  }

  async postUser(payload: User): Promise<User> {
    return await this.userRepository.create<UserModel>(payload).then(user => {
      return new User(modelToJson(user));
    }).catch(error => {
      throw new HttpException('Parametros invalidos, ou incompletos', HttpStatus.BAD_REQUEST, { cause: error });
    });
  ...
  async getUsers(query: UserModelWhere): Promise<User[]> {
    return await this.userRepository.findAll<UserModel>({ where: {
      ...query
    }}).then(users => {
  ...
  async deleteUser(id: number): Promise<User> {
    return await this.userRepository.findByPk<UserModel>(id).then((user) => {
      if (user !== null) {
        return user.destroy()
          .then(_ => {
            return new User(modelToJson(user));
  ...
  async updateUser( payload: User, idUser: number): Promise<User> {
    return await this.userRepository.findByPk<UserModel>(idUser).then((user) => {
      if (user == null) {
        throw new HttpException('Usuario por este Id nao encontrado', HttpStatus.BAD_REQUEST);
      } else {
